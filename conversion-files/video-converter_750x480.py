...
print("Encoding specs: 750x480 resolution, H.264 baseline (Pi-optimized)")  # Resolution specified here
...
for input_file in video_files:
    ...
    if ROTATE_90_DEGREES:
        ...
        # After rotation (320x480), scale to height 480 (width 320), then scale to 1280x480 and crop left/right
        filter_chain.append("scale=1280:480")  # Scale to larger width for cropping
        filter_chain.append("crop=750:480:(iw-750)/2:0")  # Crop left/right to 750
        filter_chain.append("setdar=25:16")  # Force display aspect ratio
        # For 800x480, replace above with:
        # filter_chain.append("scale=1280:480")
        # filter_chain.append("crop=800:480:(iw-800)/2:0")
        # filter_chain.append("setdar=5:3")
    else:
        # No rotation (480x320), scale to width 750 (height 500) and crop top/bottom
        filter_chain.append("scale=750:-2")  # Scale to width 750
        filter_chain.append("crop=750:480:0:(ih-480)/2")  # Crop top/bottom to 480
        filter_chain.append("setdar=25:16")  # Force display aspect ratio
        # For 800x480, replace above with:
        # filter_chain.append("scale=800:-2")
        # filter_chain.append("crop=800:480:0:(ih-480)/2")
        # filter_chain.append("setdar=5:3")
    ...
    encode_command = (
        ...
        f'-aspect 750:480 '  # Force aspect ratio for compatibility
        f'"{output_file}"'
    )
